...
2. Use Doubao Search first for normal evidence retrieval because its listed search-call price is lower. The current implementation uses Volcano Ark Responses API with `web_search` and defaults to `doubao-seed-2-0-mini-260428`; `DOUBAO_MODEL` remains configurable for another authorized Seed model after price and availability checks.
