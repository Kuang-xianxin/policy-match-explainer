...
项目已经部署上线。读者、评审或演示用户建议优先访问线上站点体验完整流程，本地部署主要用于开发、调试和二次部署。

- 线上地址：http://119.91.117.232:15173/
- 登录 / 注册：http://119.91.117.232:15173/login
- 企业画像：http://119.91.117.232:15173/profile
- 匹配结果和报告：http://119.91.117.232:15173/results

推荐体验路径：

1. 打开线上登录页并注册账号。
2. 进入企业画像页，手动填写企业信息，或输入企业名称检索候选企业。
3. 确认企业主体和画像字段，补充营收、纳税、研发、项目预算等非公开字段。
4. 保存画像并发起政策匹配。
5. 在匹配结果页查看规则基线、AI 复核、最终结论。
6. 点击生成综合评估报告，查看申报优先级、材料清单和风险提示。
...
线上体验不需要本地环境。只有在你需要开发、调试或二次部署时，才需要准备下面的本地环境：
...
如果只是体验功能，请优先使用上面的线上地址。本地启动主要面向开发者，用于修改代码、调试接口、补充政策数据或二次部署。
...
下面流程以线上站点为主。本地开发时，把地址替换为 `http://localhost:5173` 即可。

1. 打开 `http://119.91.117.232:15173/login`。
2. 注册一个新账号并登录。
3. 进入企业画像页。
