...
当前 MVP 的企业画像采用“AI 辅助轻量生成 + 用户确认”的方式。用户输入企业名称或简称后，DeepSeek 负责生成规范化查询词；后端从本地龙华企业示例索引中返回候选企业；用户选择候选企业后，系统生成一份待确认画像草稿。若没有命中本地/授权企业索引，系统会生成明确标记为 `inferred` 的未验证画像草稿，信用代码使用 `UNCONFIRMED-*`，用于继续试算匹配，不代表已经完成工商核验。
